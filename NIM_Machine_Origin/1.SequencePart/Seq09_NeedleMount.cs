...
namespace MachineControlBase
{
    ...
    public class Seq09_NeedleMount : ISequence, ISeqNo
    {
        ...
        public void Start()
        {
            ...
            // 시퀀스 시작 전 bHolderNeedlePinWorkCount를 MapData에 반영
            // 재시작 후 저장 버튼 없이 운전해도 스킵 설정이 항상 올바르게 동작하도록 보장
            ApplyNeedlePinWorkCountToMapData();
        ...
        /// <summary>
        /// bHolderNeedlePinWorkCount 설정을 MPC1_NEEDLE_MOUNT MapData에 반영한다.
        /// false인 홀은 NONE으로 세팅하여 시퀀스가 해당 홀을 건너뛰도록 한다.
        /// </summary>
        private void ApplyNeedlePinWorkCountToMapData()
        {
            MapDataLib needleMap = ml.cRunUnitData.GetIndexData(eData.MPC1_NEEDLE_MOUNT);
            bool[] pinWorkCount = ml.cSysOne.bHolderNeedlePinWorkCount;

            for (int i = 0; i < pinWorkCount.Length; i++)
            {
                if (pinWorkCount[i] == false)
                {
                    needleMap.GetUnitNo(i).eStatus = eStatus.NONE;
                }
            }
        }
                            ...
                            if (ml.cVar.bHolderNeedlePushBegin == true)
                            {
                                if (cIO.GetInput((int)eIO_I.NEEDLE_PUSHER_UP_ON) == true)
                                {
                                    ...
                                    for (int i = 0; i < ml.cRunUnitData.GetIndexData(eData.MPC1_NEEDLE_MOUNT).iMaxXindex; i++)
                                    {
                                        ...
                                        {
                                            ml.cRunUnitData.GetIndexData(eData.MPC1_NEEDLE_MOUNT).GetUnitNo(i).eStatus = eStatus.NEEDLE_MOUNT;
                                            break;
                                        }
                                    }
                                    SubNext(100);
        ...
        public bool NeedleMount()
        {
            ...
            switch (iSubStep)
            {
                ...
                case 100:   // 니들 Mounter 맵데이터 변경
                    {
                        NPC1_NeedleMount.GetUnitMin(eStatus.HOLDER).eStatus = eStatus.NEEDLE_MOUNT;
                        NeedleMounter.SetAllStatus(eStatus.EMPTY);
