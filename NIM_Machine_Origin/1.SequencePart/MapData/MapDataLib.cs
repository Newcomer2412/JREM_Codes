...
namespace MachineControlBase
{
    ...
    public static class MapDataFunction
    {
        ...
        public static readonly int[] iHolderPicker = new int[3] { 1, 3, 11 };
        public static readonly int[] iMPC_FrontFarLeft = new int[5] { 0, 1, 3, 5, 11 };
        public static readonly int[] iMPC_BUFFER1 = new int[2] { 1, 5 };
        public static readonly int[] iMPC_PipeMount = new int[4] { 1, 5, 7, 13 };
        public static readonly int[] iMPC_BUFFER2 = new int[4] { 1, 5, 7, 13 };
        public static readonly int[] iMPC_NeedleMount = new int[4] { 1, 5, 7, 13 };
        public static readonly int[] iMPC_Flip = new int[5] { 1, 7, 8, 13, 14 };
        public static readonly int[] iMPC_Disp = new int[4] { 1, 8, 9, 14 };
        public static readonly int[] iMPC_FarRight = new int[3] { 0, 1, 9 };
        public static readonly int[] iMPC_UV = new int[4] { 0, 1, 9, 11 };
        public static readonly int[] iMPC_RearFarLeft = new int[3] { 0, 1, 11 };
        public static readonly int[] iTransfer = new int[4] { 1, 2, 3, 4 };
        public static readonly int[] iWork = new int[3] { 1, 3, 4 };
        public static readonly int[] iDispenser = new int[2] { 2, 10 };
    ...
    public class MapDataLib : ICloneable
    {
        ...
        public void SetAllStatus(eStatus eStatus, bool bNoneOK = false)
        {
            ...
            if (strDataName == Enum.GetName(typeof(eData), eData.MPC1_NEEDLE_MOUNT))
            {
                ...
                foreach (BaseMapData baseMapData in BaseMapDataList)
                {
                    if (CMainLib.Ins.cSysOne.bHolderNeedlePinWorkCount[m] == true)
                    {
                        baseMapData.eStatus = eStatus;
                    }
                    else
        ...
        public void MPC_DataCopy(MapDataLib mapDataLib)
        {
            if (strDataName == Enum.GetName(typeof(eData), eData.MPC1_NEEDLE_MOUNT))
            {
                ...
                else if (GetStatus(eStatus.NEEDLE_MOUNT) == true &&
                    GetStatus(eStatus.EMPTY) == false)
                {
                    mapDataLib.SetAllStatus(eStatus.NEEDLE_MOUNT);
                }
                else
                {
            ...
            // 데이터 시프트 시, 작업할 니들 홀 데이터 오염되지 않도록 수정
            else if (strDataName == Enum.GetName(typeof(eData), eData.MPC1_BUFFER_2))
            {
                for (int i = 0; i < BaseMapDataList.Count; i++)
                {
                    ...
                    // 1. UI에서 니들 작업을 하지 않기로 설정한 홀(false)인 경우 NONE 처리하여 작업 스킵
                    if (CMainLib.Ins.cSysOne.bHolderNeedlePinWorkCount[i] == false)
                    {
                        cTargetPoint.eStatus = eStatus.NONE;
                    }
                    // 2. 작업해야 하는 홀일 경우 이전 상태에 맞춰 HOLDER로 변경 (Seq09_NeedleMount가 동작하도록)
                    else if (baseMapData.eStatus == eStatus.HOLDER)
                    {
                        cTargetPoint.eStatus = eStatus.HOLDER;
                    }
                    // 3. 그 외의 에러 상태(EMPTY, SKIPPED 등)는 그대로 복사
                    else
                    {
                        cTargetPoint.eStatus = baseMapData.eStatus;
                    }
