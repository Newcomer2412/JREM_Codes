    ...
    public class Seq08_PipeMount : ISequence, ISeqNo
    {
                ...
                case 0:
                    {
                        ...
                        if (PipeTransfer != null && PipeMounter != null && NPC1_PipeMount != null)
                        {
                            ...
                            bool bPipeMountMapData = PipeMounter.GetStatus(eStatus.MOUNT) &&
                                                     NPC1_PipeMount.GetStatus(eStatus.FIRST_HOLE);
        ...
        public bool PipeClampUp()
        {
            ...
            switch (iSubStep)
            {
                ...
                case 16:    // 파이프 트렌스퍼 R축이 현재 Posture 위치인지 확인
                    {
                        ...
                        double dTransfer_R_180_Posture = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_ROTATE, (int)eAxisPipeRotate.Rotate180);
                        if (dGetCmdTransfer_R == dTransfer_R_Posture ||
                            dGetCmdTransfer_R >= dTransfer_R_Posture + dTransfer_R_180_Posture)
                        {
                            cTimeOut.Restart();
                            SubNext(20);
                        }
                ...
                case 20:    // X, Y축 파이프 클램프 할 위치로 이동
                    {
                        double dPipe_X_ClampUp = 0.0; double dPipe_Y_ClampUp = 0.0;
                        dPipe_X_ClampUp = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_MOUNT_X, (int)eAxisPipeMount_X.PipeClampUp);
                        dPipe_Y_ClampUp = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_MOUNT_Y, (int)eAxisPipeMount_Y.PipeClampUp);

                        // Pipe Rotation 모터가 180도 회전한 상태의 위치값이면 X, Y 픽업 포지션을 변경한다.
                        double dTransfer_R = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_ROTATE, (int)eAxisPipeRotate.Rotate90);
                        double dPipeRotation = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_ROTATE, (int)eAxisPipeRotate.Rotate180);
                        double dGetCmdPipeRotate = ml.Axis[eMotor.PIPE_ROTATE].GetCmdPostion();
                        if (dGetCmdPipeRotate >= dTransfer_R + dPipeRotation)
                        {
                            dPipe_X_ClampUp = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_MOUNT_X, (int)eAxisPipeMount_X.PipeClampUp180);
                            dPipe_Y_ClampUp = ml.cAxisPosCollData.GetAxisPosition(eMotor.PIPE_MOUNT_Y, (int)eAxisPipeMount_Y.PipeClampUp180);
                        }

                        if (ml.Axis[eMotor.PIPE_MOUNT_X].MoveAbsolute(dPipe_X_ClampUp) == true &&
                            ml.Axis[eMotor.PIPE_MOUNT_Y].MoveAbsolute(dPipe_Y_ClampUp) == true)
                        {
                            cTimeOut.Stop();
                            SubNext(21);
                        }
                        else
                        {
                            ...
                            {
                                cTimeOut.Stop();
                                if (ml.Axis[eMotor.PIPE_MOUNT_X].MoveAbsolute(dPipe_X_ClampUp) == false)
                                    ml.AddError(eErrorCode.MOTOR12_MOVE_TIMEOUT, iSubStep);
                                else if (ml.Axis[eMotor.PIPE_MOUNT_Y].MoveAbsolute(dPipe_Y_ClampUp) == false)
                ...
                case 22:    // Z축 클램프 할 위치로 이동
                    {
                        ...
                        {
                            cIO.SetOutput((int)eIO_O.PIPE_CLAMP_VACUUM, true);
                            cTimeOut.Restart();
                            SubNext(23);
                        }
                        else
                ...
                case 26:    // 파이프 푸쉬할 실린더 Up 진행 후 200ms 대기 후 진행한다.
                    {
                        if (cDelay.ElapsedMilliseconds > 200)
                        {
                            cDelay.Stop();
                ...
                case 30:    // Y축을 파이프가 진공에 가까워 지도록 이동시킨다.
                    {
                        if (ml.Axis[eMotor.PIPE_MOUNT_Y].MoveRelative((int)eAxisPipeMount_Y.ClampVacuumPitch, false) == true)
                        {
                            cTimeOut.Stop();
                            SubNext(31);
                        }
                ...
                case 41:    // Z축이 올라가는 중에 니들 Chuck 클로즈
                    {
                        if (ml.Axis[eMotor.PIPE_MOUNT_Z].GetActPostion() >= -20)
                        {
                        ...
                        if (ml.Axis[eMotor.PIPE_MOUNT_Z].IsMoveDone() == true)
                        {
                            if (ml.cVar.bHolderPipePushBegin == true)
                            {
                                if (cIO.GetInput((int)eIO_I.PIPE_PUSHER_UP_ON) == true)
                                {
                                    ...
                                    {
                                        if (ml.cRunUnitData.GetIndexData(eData.MPC1_PIPE_MOUNT).GetUnitNo(i).eStatus == eStatus.FIRST_HOLE)
                                        {
                                            ml.cRunUnitData.GetIndexData(eData.MPC1_PIPE_MOUNT).GetUnitNo(i).eStatus = eStatus.FIRST_MOUNT;
                                            break;
                                        }
                                    }
                                    SubNext(100);
        ...
        public bool PipeMount()
        {
            ...
            switch (iSubStep)
            {
                ...
                case 23:    // 촬영 완료
                    {
                        if (CAM3_ResultData.bShootFinish == true)
                        {
                            ...
                            {
                                cTimeOut.Stop();
                                NPC1_PipeMount.GetUnitMin(eStatus.FIRST_HOLE).eStatus = eStatus.SKIPPED;
                ...
                case 30:    // X, Y축 파이프 마운트 보정 좌표이동
                    {
                        ...
                        {
                            NLogger.AddLog(eLogType.Seq08_PipeMount, NLogger.eLogLevel.INFO, $"Move Pipe Mount : X:{dMovePipeMount_X} Y:{dMovePipeMount_Y}");
                            cTimeOut.Stop();
                            SubNext(31);
                        }
                        else
                ...
                case 51:    // 파이프 마운트 X, Y축 홀더 Push 위치 이동 확인
                    {
                            ...
                            ml.Axis[eMotor.PIPE_MOUNT_Y].IsMoveDone() == true)
                        {
                            ...
                            if (ml.cRunUnitData.GetIndexData(eData.PIPE_TRANSFER).GetStatus(eStatus.WORK_DONE) == true &&
                                NPC1_PipeMount.GetStatusCount(eStatus.FIRST_HOLE) > 1)
                            {
                ...
                case 100:   // 파이프 마운터 맵데이터 변경
                    {
                        NPC1_PipeMount.GetUnitMin(eStatus.HOLDER).eStatus = eStatus.FIRST_MOUNT;
                        PipeMounter.SetAllStatus(eStatus.EMPTY);
