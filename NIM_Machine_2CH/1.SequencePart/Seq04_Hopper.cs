    ...
    public class Seq04_Hopper : ISequence, ISeqNo
    {
                ...
                case 0:
                    {
                        ...
                        if (PipePnP != null && NeedlePnP != null && FeederBasket != null)
                        {
                            // 호퍼 동작 맵데이터 조건 확인
        ...
        public bool HopperRun()
        {
            ...
            switch (iSubStep)
            {
                ...
                case 21:    // 파이프 또는 니들이 놓여진 개수에 따라 Hopper에서 모듈을 공급
                    {
                        ...
                        {
                            cTimeOut.Stop();
                            // 파이프와 니들 공급할 필요가 없으면 시퀀스를 종료
                            if (CAM0_ResultData.uiPipeCount >= cSysArray.uiFeederPipeCount &&
                                CAM0_ResultData.uiNeedleCount >= cSysArray.uiFeederNeedleCount ||
                                Define.SIMULATION == true)
                            {
                                ClearBlow_BackLight();
                                SubNext(100);
                ...
                case 31:    // 파이프 또는 니들이 놓여진 개수에 따라 Hopper에서 모듈을 공급
                    {
                        ...
                        {
                            cTimeOut.Stop();
                            // 파이프와 니들 모두 공급할 필요가 없으면 시퀀스를 종료
                            if (CAM0_ResultData.uiPipeCount >= cSysArray.uiFeederPipeCount &&
                                CAM0_ResultData.uiNeedleCount >= cSysArray.uiFeederNeedleCount ||
                                Define.SIMULATION == true)
                            {
                            ...
                            // 둘다 공급해야 할때
                            else if (CAM0_ResultData.uiPipeCount < cSysArray.uiFeederPipeCount &&
                                     CAM0_ResultData.uiNeedleCount < cSysArray.uiFeederNeedleCount)
                            {
                                // 파이프 찾기 실패
                                if (iPipeFindCount >= 5)
                                {
                                    ClearBlow_BackLight();
                                    ml.AddError(eErrorCode.SEQ04_FEEDER_NOT_FIND_PIPE, iSubStep);
                                    return true;
                                }
                                // 니들 찾기 실패
                                else if (iNeedleFindCount >= 5)
                                {
                                    ClearBlow_BackLight();
                            ...
                            else if (CAM0_ResultData.uiPipeCount < cSysArray.uiFeederPipeCount)
                            {
                                ...
                                {
                                    iPipeFindCount++;
                                    HopperMotor = eMotor.PIPE_HOPPER;
                                    uiHopperDelay = cSysArray.uiHopperPipeRunDelay;
                                    SubNext(40);
                                }
                            }
                            // 니들이 공급해야 할때
                ...
                case 51:    // 딜레이만큼 Hopper를 동작 후 정지한다.
                    {
                        ...
                        // 모든 공급을 마치면 다시 파이프와 니들의 개수를 파악하기 위해 비전 촬영을 한다.
